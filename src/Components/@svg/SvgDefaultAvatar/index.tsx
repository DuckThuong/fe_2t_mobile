import React from 'react';

export const SvgDefaultAvatar = () => (
  <span>
    <svg
      width="52"
      height="52"
      viewBox="0 0 52 52"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
    >
      <rect width="52" height="52" fill="url(#pattern0_570_668)" />
      <defs>
        <pattern id="pattern0_570_668" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlinkHref="#image0_570_668" transform="scale(0.015625)" />
        </pattern>
        <image
          id="image0_570_668"
          width="64"
          height="64"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  </span>
);
